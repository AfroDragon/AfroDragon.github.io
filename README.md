...
<body class="h-screen overflow-hidden flex flex-col text-sm md:text-base">
        ...
        <section class="flex-1 flex flex-col bg-gray-800 relative z-0">
            ...
            <div class="flex-1 flex flex-col items-center pt-4 p-4 md:p-8 relative overflow-y-auto custom-scrollbar bg-gray-800/50" id="battle-area">
                ...
                <div class="flex flex-col items-center w-full max-w-md relative z-10">
                    ...
                    <!-- Buff Bar -->
                    <div id="buff-container" class="w-full flex gap-2 justify-center min-h-[32px] mb-2 flex-wrap">
                        <!-- Buffs inserted here -->
                    </div>

                    <div class="w-full flex justify-between text-xs text-gray-400 mb-1 font-bold">
                        <span class="text-green-300">Hero</span>
                        <span>HP</span>
                    </div>
    ...
    <script>
        ...
        const ELEMENTAL_CHART = {
            ...
            'holy': { strong: 'darkness', weak: 'darkness' },
            'darkness': { strong: 'holy', weak: 'holy' },
            'physical': { strong: null, weak: null }
        };

        const BUFF_ICONS = {
            'warCry': { icon: 'fa-bullhorn', color: 'bg-red-800 border-red-500' },
            'defensiveStance': { icon: 'fa-shield-halved', color: 'bg-green-800 border-green-500' },
            'immortality': { icon: 'fa-infinity', color: 'bg-yellow-600 border-yellow-300' },
            'doubleStrike': { icon: 'fa-xmarks-lines', color: 'bg-orange-800 border-orange-500' },
            'galeBuff': { icon: 'fa-wind', color: 'bg-slate-700 border-slate-400' },
            'blessing': { icon: 'fa-hands-praying', color: 'bg-yellow-800 border-yellow-500' },
            'curse': { icon: 'fa-skull', color: 'bg-purple-900 border-purple-500' },
            'confusion': { icon: 'fa-face-dizzy', color: 'bg-pink-900 border-pink-500' },
            'webSlow': { icon: 'fa-spider', color: 'bg-gray-700 border-gray-500' },
            'enemyWet': { icon: 'fa-droplet', color: 'bg-blue-900 border-blue-500' }
        };
        ...
        const ARTIFACT_DATA = {
            ...
            // Zone 3: Molten Core
            "Magma Cube": { name: "Obsidian Shard", icon: "game-icons:rock", desc: "Sharp volcanic glass." },
        ...
        const SKILLS = {
            strength: [
                { id: 's1', name: 'Lacerate', desc: 'Deal Damage/sec', cost: 1, type: 'passive', effect: { dpsPct: 0.10 } },
            ...
            dexterity: [
                ...
                { id: 'd4', name: 'Double Strike', desc: 'Active: Attacks do 150% Dmg', cost: 2, type: 'active', base: 5000, cooldown: 25000, effect: 'doubleStrikeBuff', element: 'physical', scale: { duration: 1000 } },
                { id: 'd5', name: 'Evasion', desc: '+2.5% Dodge Chance', cost: 3, type: 'passive', effect: { dodge: 0.025 } },
                { id: 'd6', name: 'Omnislash', desc: 'Active: Multi-hit Attack', cost: 4, type: 'active', base: 3, cooldown: 20000, effect: 'omnislash', element: 'physical', scale: { hits: 0.5 } }
            ],
            intelligence: [
                ...
                { id: 'i3', name: 'Multitasking', desc: '+2% Cooldown Reduction', cost: 2, type: 'passive', effect: { cdr: 0.02 } },
                ...
                { id: 'i5', name: 'Neural Network', desc: 'Auto-trigger +1 Active Skill', cost: 3, type: 'passive', effect: { autocastSlots: 1 } },
            ...
            darkness: [
                ...
                { id: 'da2', name: 'Drain Life', desc: 'Active: Steal HP', cost: 1, type: 'active', base: 0.2, cooldown: 8000, effect: 'drainLife', element: 'darkness', scale: { drainPct: 0.01 } },
        ...
        function getDynamicSkillDesc(skill, currentLevel) {
            ...
            const isNext = true; // Always show next step for upgrades
            ...
            if (skill.type === 'passive') {
                ...
                // ... Generic catch-all for simple passives
                else {
                    return skill.desc;
                }
            }
            ...
            // --- ACTIVE SKILLS ---
                ...
                const getVal = (l) => {
                    ...
                    if (skill.scale && skill.scale.hits) {
                        // Special logic for Omnislash hits: Base + floor((lvl-1)*scale)
                    ...
                    if (skill.scale && skill.scale.intScale) {
                        // Base damage logic for magic bullet is flat Base + (INT * Scale)
                        // Here we just show the Scale factor
                        let scaleFactor = 5 + ((l-1) * skill.scale.intScale);
                        return { type: ' + (INT x Scale)', val: scaleFactor };
                    }

                    // Fallbacks
                    if (skill.effect === 'ignite') return { type: ' Burn Dmg', val: base + l };
                ...
                if (skill.id === 's4') descCurrent = `Effect: +50% Damage<br>${descCurrent}`;
                if (skill.id === 'e4') descCurrent = `Effect: -50% Dmg Taken<br>${descCurrent}`;
                if (skill.id === 'e6') descCurrent = `Effect: Invulnerable<br>${descCurrent}`;
                if (skill.id === 'd4') descCurrent = `Effect: 1.5x Damage<br>${descCurrent}`;
                if (skill.id === 'a4') descCurrent = `Effect: 2x Attack Speed<br>${descCurrent}`;
                if (skill.id === 'h4') descCurrent = `Effect: Heal + 20% Dmg<br>${descCurrent}`;
                
                // Added for User Request
                if (skill.id === 'da4') descCurrent = `Effect: +50% Dmg Taken<br>${descCurrent}`; // Curse
                if (skill.id === 'da6') descCurrent = `Effect: Enemy attacks self<br>${descCurrent}`; // Nightmare
                if (skill.id === 'ea4') descCurrent = `Effect: Stuns Enemy<br>${descCurrent}`; // Tremor
                if (skill.id === 'ic4') descCurrent = `Effect: Stuns Enemy<br>${descCurrent}`; // Freeze
                if (skill.id === 'l4') descCurrent = `Effect: Stuns Enemy<br>${descCurrent}`; // Paralysis
                if (skill.id === 'p4') descCurrent = `Effect: Slows Atk Speed<br>${descCurrent}`; // Entangle
                if (skill.id === 'w4') descCurrent = `Effect: Slows Atk Speed<br>${descCurrent}`; // Chill
                if (skill.id === 'i4') descCurrent = `Effect: Reduced cooldowns<br>${descCurrent}`; // Time Warp

                // Only show increase if it actually increased (for integer steps like Omnislash)
                if (nxt.val != cur.val) {
                    let diff = nxt.val - cur.val;
                    if(typeof nxt.val === 'number') diff = Number(diff.toFixed(1));
                    descNext = `Next: ${nxt.val}${cur.type}`;
                } else {
                     descNext = `Next: No change (Scale pending)`;
                }
            }
        ...
        function gameLoop(timestamp) {
            ...
            if (combat.isBoss && combat.currentGimmick) {
                ...
                // Frozen Heart: Slow
                // Effect applied in attackSpeed calc
            }
            ...
            combat.attackTimer = Math.min(combat.attackTimer + delta, attackInterval);
            ...
            // Blizzard Passive Logic (Ice Ult)
            const blizzard = activeEffects.find(e => e.type === 'blizzard');
            if (blizzard) {
                // Handled in activeEffects loop
            }
            ...
            if (element !== 'physical' && enemyElem !== 'physical') {
                ...
                if (chart) {
                    ...
                    if (chart.strong === enemyElem || (Array.isArray(chart.strong) && chart.strong.includes(enemyElem))) {
                        ...
                        multiplier = 1.5 + ((elementLevel - 1) * 0.10);
        ...
        function playerAttack() {
            ...
            if (combat.buffs.find(b => b.id === 'doubleStrike')) dmg *= 1.5;
        ...
        function castActive(skillId, isAutocast = false) {
            ...
            const addBuff = (id, baseDur) => {
                ...
                if (existing) {
                    ...
                    combat.buffs.push({ id: id, duration: dur });
                }
                updateBuffIcons();
            };
            ...
            switch(skillDef.effect) {
                ...
                case 'doubleStrikeBuff':
                    ...
                    spawnFloatingText("Power Up!", 'player-hp-bar', 'text-yellow-300');
                    break;
                case 'omnislash':
        ...
        function updateBuffIcons() {
            ...
            combat.buffs.forEach(buff => {
                const meta = BUFF_ICONS[buff.id] || { icon: 'fa-star', color: 'bg-gray-600 border-gray-400' };
                
                const el = document.createElement('div');
                el.className = `buff-icon w-8 h-8 rounded-full border-2 text-white flex items-center justify-center relative shadow-lg ${meta.color}`;
                el.innerHTML = `
                    <i class="fa-solid ${meta.icon} text-xs"></i>
                    <div class="absolute -bottom-1 -right-1 text-[8px] bg-black text-white px-1 rounded-full border border-gray-600 font-mono">${Math.ceil(buff.duration/1000)}s</div>
                `;
                container.appendChild(el);
            });
        }
